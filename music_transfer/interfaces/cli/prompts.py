...
Migrated unchanged in behaviour from the legacy ``ui/prompts.py``: destructive
actions still require a yes/no decision *and* typing the exact ``DELETE``
phrase.  Only the import path changed.
"""
...
class Prompts:
    ...
    def choose_values(self, prompt_key: str, options: Mapping[str, str]) -> str:
        """Choose dynamic display values, such as locally discovered job ids."""
    ...
    def confirm_plan(self, view_total: int) -> bool:
        """Confirm a transfer plan before anything is written."""

        self._console.message("confirmation.warning_title", style="warning")
        self._console.message(
            "confirmation.action",
            action=self._i18n.t("confirmation.transfer_action", count=view_total),
        )
        self._console.message("confirmation.mutates_account", style="warning")
        return self.yes_no("plan.confirm")
    ...
    def confirm_deletion(self, account: str, summary: str) -> bool:
        ...
        self._console.message(
            "confirmation.type_phrase",
            phrase=self._i18n.t("confirmation.delete_phrase"),
        )
